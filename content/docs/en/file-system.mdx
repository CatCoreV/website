...
`/app`, `/bin`, `/boot`, `/config`, `/data`, `/dev`, `/lib`, `/system` and `/tmp` are **mounts**, each one has it's own use:

<Accordions>
  <Accordion title="/app" id="filesystem-app">
    `/app` mount contains a **APP**lication's embedded files. Applications can have embedded files (for example, .png icons) packaged directly into the .app file, and those files can be accessed in `/app` mount. Remember that this mount is read-only, you can't modify an .app file using it.
  </Accordion>
  <Accordion title="/bin" id="filesystem-bin">
    `/bin` mount contains **BIN**aries of applications (.app files). If user tries to start an application by specifying it's name, it's a standard to look for it in there, even though it's not really required (you're able to launch applications from a different folder instead of `/bin`).
  </Accordion>
  ...
  <Accordion title="/config" id="filesystem-config">
  ...
  <Accordion title="/data" id="filesystem-data">
    `/data` is empty by default. You're supposed to store anything your OS/user/apps want there.
  </Accordion>
  <Accordion title="/dev" id="filesystem-dev">
    `/dev` contains **devices** inside and it's contents may vary on the host device.
  </Accordion>
  <Accordion title="/lib" id="filesystem-lib">
    `/lib` mount contains **LIB**aries for applications to use. By default of `lib_path` (from `/config/kernel.conf`), libraries will first be checked in `/app`, and then in `/lib`, but it's not really required (you're able to change `lib_path` to use a different folder instead of `/lib`).
  </Accordion>
  <Accordion title="/system" id="filesystem-system">
  ...
  <Accordion title="/tmp" id="filesystem-tmp">
    `/tmp` is a temporary in-memory file system. You're able to read and write any files there, but they'll be stored in RAM only, which means they will not stay after a reboot.
  </Accordion>
</Accordions>
